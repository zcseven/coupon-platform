...
func (m *defaultUserInfoModel) Insert(ctx context.Context, data *UserInfo) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.UserName, data.UserTelephone, data.IsActive)
	return ret, err
}
...
func (m *defaultUserInfoModel) Update(ctx context.Context, data *UserInfo) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.UserName, data.UserTelephone, data.IsActive, data.Id)
	return err
}
